...
VSX.prototype.selectInput = function (input) {
  ...
  debug_log(
    '(pioneer-avr::VSX.prototype.selectInput) set input mode to: ' + input + ', type: ' + typeof input + ', cmd: ' + cmd
  );
...
function handleData(self, d) {
  ...
  if (data.startsWith('PWR')) {
    ...
    // listening mode     // DO NOT REACT ON SR - IT REFLECTS THE REMOTE KEY PRESSED TO CYCLIC SHIFT THROUGH MODES
    ...
    var mode_str = null;
    for (var ms in ListeningModes)
      if (ListeningModes[ms][1].includes(mode_hex)) {
    ...
    if (mode_str != null)
      debug_log(
        '(pioneer-avr::handleData) decoded hex listening mode: 0x' +
          mode_hex.toString(16) +
          ' into ' +
          mode_int +
          ' (' +
          mode_str +
