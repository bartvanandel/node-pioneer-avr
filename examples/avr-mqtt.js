...
const AvrMqtt = function (options) {
  const self = this;
...
AvrMqtt.prototype._handleAvrConnect = function (options) {
  ...
  self.receiver.on('power', function (power) {
    ...
    self.mqttClient.publish(self.TOPIC_powerOut, JSON.stringify(self.state.power));
  });
  self.receiver.on('volume', function (db) {
    self.state.volume = { value: db, unit: 'dB' };
    self.mqttClient.publish(self.TOPIC_volumeOut, JSON.stringify(self.state.volume));
  });
  self.receiver.on('mute', function (mute) {
    self.state.mute = { value: mute };
    self.mqttClient.publish(self.TOPIC_muteOut, JSON.stringify(self.state.mute));
  });
  self.receiver.on('input', function (source, name) {
    self.state.source = { value: source, name };
    self.mqttClient.publish(self.TOPIC_sourceOut, JSON.stringify(self.state.source));
  });
  self.receiver.on('inputName', function (id, name) {
    ...
    self.mqttClient.publish(self.TOPIC_sourcesOut, JSON.stringify(self.state.sources));
    ...
    if (self.state.source && self.state.source.value === id) {
      self.state.source = { value: id, name };
      self.mqttClient.publish(self.TOPIC_sourceOut, JSON.stringify(self.state.source));
    }
  });
};
...
AvrMqtt.prototype._handleContentRequest = function (topic, payload) {
  const self = this;
  const i = topic.indexOf('?');
  const requestTopic = topic.slice(0, i);
...
AvrMqtt.prototype._handleInput = function (topic, payload) {
  ...
  if (topic === self.TOPIC_powerIn) {
    ...
    const msg = JSON.parse(payload);
    self.receiver.changeInput(msg.value);
  }
